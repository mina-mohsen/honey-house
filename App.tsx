...
const App: React.FC = () => {
  ...
  // Admin controls for reviews (client keeps key locally)
  const [adminKey, setAdminKey] = useState<string>(() => localStorage.getItem('hh_admin_key') || '');
  const isAdmin = !!adminKey;
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editDraft, setEditDraft] = useState({ name: '', rating: 5, comment: '' });

  // Order validation refs
  const nameRef = useRef<HTMLInputElement | null>(null);
  const phoneRef = useRef<HTMLInputElement | null>(null);
  const locationRef = useRef<HTMLInputElement | null>(null);
  const [missingField, setMissingField] = useState<'name' | 'phone' | 'location' | null>(null);

  const t = useMemo(() => TRANSLATIONS[lang], [lang]);

  useEffect(() => {
    // Load reviews from server (Vercel KV). If empty/unavailable, fallback to MOCK_REVIEWS.
    (async () => {
      ...
      try {
        ...
        if (Array.isArray(data) && data.length) {
          ...
          // fallback to mock reviews shown to users until you seed KV
          setReviews(
            MOCK_REVIEWS.map((r: any, idx: number) => ({
              id: r.id || `mock-${idx}`,
              ar: r.ar,
              en: r.en,
              rating: r.rating,
    ...
    // Load admin key (if you set it before)
    const k = localStorage.getItem('hh_admin_key') || '';
    setAdminKey(k);
  }, []);
  ...
  const handleSendOrder = () => {
    // Validate and focus missing field (better UX than a generic alert)
    const focusField = (field: 'name' | 'phone' | 'location') => {
      setMissingField(field);
      const refMap = { name: nameRef, phone: phoneRef, location: locationRef } as const;
    ...
    if (!customer.name.trim()) return focusField('name');
    if (!customer.phone.trim()) return focusField('phone');
    if (!customer.location.trim()) return focusField('location');
    if (cart.length === 0) {
      alert(t.errorFields);
      return;
    }
...
Provide a very short, professional, and helpful response.`,
  ...
  const reviewsAvg = useMemo(() => {
    if (!allReviewsDisplay.length) return 0;
    return allReviewsDisplay.reduce((acc: number, r: any) => acc + (Number(r.rating) || 0), 0) / allReviewsDisplay.length;
  }, [allReviewsDisplay]);

  const reviewsCount = allReviewsDisplay.length;
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-3xl border-b border-amber-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 flex items-center gap-4 md:gap-8 overflow-x-auto no-scrollbar">
          ...
          <nav className="flex items-center gap-1 md:gap-2 py-1">
            ...
            <NavBtn icon="🍯" label={t.navSelections} onClick={() => { setOpenProducts(true); scrollToSection('products-grid'); }} />
            <NavBtn icon="⭐" label={t.navReviews} onClick={() => { setOpenReviews(true); scrollToSection('reviews-section'); }} />
...
{showReviewForm && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
             {allReviewsDisplay.map((r, i) => (
               <div key={r.id || i} className="bg-white p-7 sm:p-10 rounded-3xl border border-amber-50 shadow-sm relative group hover:shadow-xl hover:-translate-y-1 transition-all">
                  <div className="flex items-center justify-between gap-3 mb-5">
                    <div className="flex items-center gap-1">
                      {[...Array(r.rating)].map((_, si) => <span key={si} className="text-amber-400 text-xl">★</span>)}
                    </div>
                    ...
                    {isAdmin && (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          type="button"
                          onClick={async () => {
                            if (!confirm(lang === 'ar' ? 'متأكد تمسح التقييم؟' : 'Delete this review?')) return;
                            try {
                          ...
                          className="px-3 py-2 rounded-xl bg-red-50 border border-red-100 text-red-600 text-xs font-black hover:bg-red-100"
                        >
                    ...
                    <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 sm:p-5 space-y-3">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <select
                          ...
                          onChange={e => setEditDraft(d => ({ ...d, rating: Number(e.target.value) }))}
                          className="w-full p-3 rounded-xl border border-slate-200 font-bold"
                        >
                      ...
                      <textarea
                        ...
                        className="w-full p-3 rounded-xl border border-slate-200 font-bold"
                        rows={4}
                        placeholder={lang === 'ar' ? 'التعليق' : 'Comment'}
                      />
                      ...
                      <div className="flex gap-2">
                          ...
                          onClick={async () => {
                            try {
                              const res = await fetch('/api/reviews', {
                                ...
                                body: JSON.stringify({ id: r.id, name: editDraft.name, rating: editDraft.rating, comment: editDraft.comment, lang }),
                              });
                        ...
                        <button
                          ...
                          className="px-4 py-2 rounded-xl bg-white border border-slate-200 font-black hover:bg-slate-50"
                        >
...
const InputGroup = ({ label, placeholder, value, onChange, id, inputRef, error, errorText }: any) => (
  <div className="space-y-3 sm:space-y-4">
    <label htmlFor={id} className="text-[10px] font-black text-amber-900/30 uppercase tracking-[0.3em] px-2">{label}</label>
    <input
      id={id}
      ref={inputRef}
      type="text"
      ...
      className={`w-full p-4 sm:p-5 bg-slate-50 rounded-2xl border outline-none transition-all placeholder:text-slate-300 font-bold text-base sm:text-lg focus:ring-8 focus:ring-amber-500/5 focus:border-amber-500 ${
        error ? 'border-red-400 ring-4 ring-red-500/10' : 'border-slate-100'
      }`}
      placeholder={placeholder}
    />
    {error && <p className="text-xs font-bold text-red-500 px-2">{errorText}</p>}
  </div>
);
